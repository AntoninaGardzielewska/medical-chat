...
class FetchArticles:
    ...
    def search_for_ids(self, term: str, max_results: int) -> List[int]:
        response = httpx.get("https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi",
                             params={
                                'db': 'pubmed',
                                'term': term,
                                'retmax': max_results,
    ...
    def fetch_by_ids(self, ids: List[int]):
        ...
        response = httpx.post('https://eutils.ncbi.nlm.nih.gov/entrez/eutils/efetch.fcgi',
                              params = {
                                  'db': 'pubmed',
                                  'retmode': 'xml',
                                  'id': ",".join(ids)
